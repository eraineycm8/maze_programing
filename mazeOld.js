  ...
  brickTexture.onload = function() {
    roadTexture.onload = function () {
      finishTexture.onload = function () {
        ...
        for (let y = 0; y < CELL_HEIGHT; y++) {
          for (let x = 0; x < CELL_WIDTH; x++) {
            if (maze[y][x] instanceof Item) {
              maze[y][x].draw(ctx); // Chama o método draw da classe Item para desenhar o item
            }
          }
        }
...
// Função para escolher aleatoriamente um item de uma matriz
function getRandomItemFromArray(array) {
  return array[Math.floor(Math.random() * array.length)];
}

// Função para marcar as posições dos itens no labirinto
function markItemPositions(maze, itemTypes, minItems, maxItems) {
  const numberOfItems = getRandomNumber(minItems, maxItems);
  const itemPositions = {};

  for (let i = 0; i < numberOfItems; i++) {
    const x = getRandomNumber(0, CELL_WIDTH - 1);
    const y = getRandomNumber(0, CELL_HEIGHT - 1);
    const itemType = getRandomItemFromArray(itemTypes);

    maze[y][x] = itemType;

    itemPositions[itemType] = itemPositions[itemType] || [];
    itemPositions[itemType].push({ x, y });
  }

  return itemPositions;
}

// Função para desenhar um item no centro de uma célula
function drawItem(ctx, itemType, x, y) {
  const centerX = MARGIN + x * CELL_SIZE + CELL_SIZE / 2;
  const centerY = MARGIN + y * CELL_SIZE + CELL_SIZE / 2;
...
// Função principal para marcar as posições dos itens e desenhá-los no labirinto
// Função para gerar um número aleatório entre min e max, inclusive
function getRandomNumber(min, max) {
  return Math.floor(Math.random() * (max - min + 1)) + min;
}
...
// Função principal para marcar as posições dos itens e desenhá-los no labirinto
// Função para marcar posições aleatórias dos itens no labirinto
// Função para marcar posições aleatórias dos itens no labirinto
function markRandomItemPositions(maze) {
  const minItems = 4;
  const maxItems = 7;
  const numberOfItems = Math.floor(Math.random() * (maxItems - minItems + 1)) + minItems;

  const itemTypes = ['life', 'health', 'key', 'energy'];

  for (let i = 0; i < numberOfItems; i++) {
    // Gerar posições aleatórias no labirinto
    let x, y;
    do {
      x = Math.floor(Math.random() * CELL_WIDTH);
      y = Math.floor(Math.random() * CELL_HEIGHT);
    } while (maze[y][x] !== 0); // Garante que o item seja colocado apenas em células vazias

    // Escolher aleatoriamente um tipo de item
    const itemType = itemTypes[Math.floor(Math.random() * itemTypes.length)];

    // Marcar a posição no labirinto com o tipo de item
    maze[y][x] = new Item(itemType, x, y);
  }
}
